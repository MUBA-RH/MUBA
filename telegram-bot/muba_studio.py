...
STUDIO_REFERENCE_FILE=Path(__file__).resolve().parent/"assets"/"muba_studio_identity.png"
STUDIO_REFERENCE_SHA256="9ad28b5756ecd79d262ba332358592c2e8635a01b7080405003682a0db2678b3"
DAILY_LIMIT=1
...
def ai_payload(prompt:str,kind:str,reference_data_uri:str)->dict:
    ...
    instruction=(f"Use the clean reference portrait only as MUBA identity guidance, not as a rigid composition template or an image to paste into a scene. Keep MUBA recognizably MUBA through the core facial identity: exceptionally large asymmetrical glossy brown eyes, small rounded brown nose, compact furry muzzle, playful open mouth and pink tongue, short dense tan-brown fur, and characteristic face-to-body proportions. Preserve the relationship of the eyes, nose, mouth and fur while allowing expression and gaze to change with the action. Redraw MUBA naturally for the user's concept, pose, outfit, framing, scale, lighting, environment and visual style: one coherent scene, matching perspective, light direction, shadows, materials and fur detail throughout. No hard cutout edges, pasted portrait, floating head, visible mask seam, mismatched lighting, collage, identity sheet, reference panels or duplicated character. Do not force a large centered MUBA portrait, circular avatar framing, black cap, or black $MUBA hoodie unless the user asks for them or they fit the scene naturally. The requested concept should lead the composition; MUBA should belong inside the scene rather than dominate it by default. {format_hint}. {_text_policy(prompt)} User request: {clean_prompt(prompt)}")
...
<h1>🎭 MUBA Studio</h1><small>MUBA's face stays recognizable; your idea shapes the scene. Daily limit: 1. Creations appear on the website.</small>
...
document.getElementById('go').onclick=async()=>{{let prompt=document.getElementById('p').value.trim();if(!prompt)return;document.getElementById('actions').style.display='none';
let go=document.getElementById('go'),progress=1,timer=null,ok=false;go.disabled=true;go.style.transition='transform .28s ease';go.style.transform='rotateX(180deg)';const draw=()=>{{go.textContent='ÜRETİM YAPILIYOR %'+progress}};draw();setTimeout(()=>{{go.style.transform='rotateX(0deg)'}},280);timer=setInterval(()=>{{if(progress>=94)return;progress=Math.min(94,progress+(progress<55?4:progress<80?2:1));draw()}},650);
try{{let r=await fetch('{b}/studio/generate',{{method:'POST',headers:{{'content-type':'application/json'}},body:JSON.stringify({{initData:tg.initData,uid:new URLSearchParams(location.search).get('uid'),studioToken:new URLSearchParams(location.search).get('st'),prompt,kind}})}});
let m=document.getElementById('msg');if(!r.ok){{m.textContent=(await r.json()).error||'Could not create.';return}}
let outputUrl=r.headers.get('X-MUBA-Output-URL');await r.blob();if(!outputUrl){{m.textContent='Could not create a shareable output.';return}}ok=true;let im=document.getElementById('preview');im.src=outputUrl;im.style.display='block';let actions=document.getElementById('actions');actions.style.display='grid';document.getElementById('download').href=outputUrl+'?download=1';document.getElementById('copy').onclick=async()=>{{try{{await navigator.clipboard.writeText(outputUrl);m.textContent='Link copied.'}}catch(e){{prompt('Copy this link:',outputUrl)}}}};
let rem=r.headers.get('X-MUBA-Remaining');let gallery=r.headers.get('X-MUBA-Gallery-Status');m.textContent=(gallery==='published'?'Created and added to MUBA Gallery.':gallery==='test'?'Test preview ready. Gallery was not changed.':'Image created, but Gallery could not save it. Download it now.')+' '+(rem==='DEV'?'DEV unlimited':rem+'/1 left today.')}}finally{{if(timer)clearInterval(timer);go.textContent=ok?'ÜRETİM TAMAMLANDI %100 ✓':'ÜRETİM BAŞARISIZ — TEKRAR DENE';await new Promise(resolve=>setTimeout(resolve,ok?700:1200));go.style.transform='';go.disabled=false;go.textContent='CREATE'}}}};
</script></body></html>"""
