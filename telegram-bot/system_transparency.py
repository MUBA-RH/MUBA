...
Each locale contains the same transparency topics, split into community platform-safe pages.
"""
...
MUBA is more than a community platform bot. The current system brings together MUBA's verified knowledge base, five-language Assistant, natural conversation layer, community tools, Guardian security layer, creative tools and public web presence.
...
MUBA Daily provides structured access to MUBA's official/public information areas such as X, the website, community platform and updates.
...
Guardian is separate from Assistant. Assistant handles information and conversation; Guardian is the security and management layer of the designated MUBA main community platform group.
...
MUBA also has a public web layer hosted from its version-control system project. The website is the public-facing home; community platform Assistant is the interactive knowledge/conversation interface; Guardian is the group-security layer.

These components belong to one ecosystem but have different responsibilities.""",
"""SYSTEM FLOW

MUBA separates public interaction, verified knowledge, creative production and community security into distinct responsibilities.

Requests are directed to the appropriate system layer while basic continuity safeguards help keep normal operation stable.

Public transparency explains responsibilities at a functional level. Internal routing, credentials, access points and security-sensitive implementation details remain private.""",
"""INFORMATION POLICY
...
The production baseline lives on the main version-control system branch. Changes are developed in isolated branches, tested, reviewed through pull requests and merged only after the required checks succeed.
...
MUBA yalnızca bir community platform botu değildir. Mevcut sistem; MUBA'nın doğrulanmış bilgi tabanını, beş dilli Assistant'ı, doğal konuşma katmanını, topluluk araçlarını, Guardian güvenlik katmanını, yaratıcı araçları ve herkese açık web varlığını bir araya getirir.
...
MUBA Daily; X, web sitesi, community platform ve güncellemeler gibi MUBA'nın resmî/herkese açık bilgi alanlarına düzenli erişim sağlar.
...
Guardian, Assistant'tan ayrıdır. Assistant bilgi ve sohbeti yönetirken Guardian, belirlenmiş MUBA ana community platform grubunun güvenlik ve yönetim katmanıdır.
...
MUBA'nın version-control system projesinden yayımlanan herkese açık bir web katmanı da vardır. Web sitesi dışarıya açık ev; community platform Assistant etkileşimli bilgi/sohbet arayüzü; Guardian ise grup güvenliği katmanıdır.

Bu bileşenler aynı ekosisteme aittir fakat sorumlulukları farklıdır.""",
"""SİSTEM AKIŞI

MUBA; kamusal etkileşim, doğrulanmış bilgi, yaratıcı üretim ve topluluk güvenliğini birbirinden ayrılmış sorumluluklar olarak yönetir.

İstekler uygun sistem katmanına yönlendirilir; temel süreklilik korumaları normal çalışmanın stabil kalmasına yardımcı olur.

Kamusal şeffaflık görevleri işlev düzeyinde açıklar. İç yönlendirme, kimlik bilgileri, erişim noktaları ve güvenlik açısından hassas uygulama ayrıntıları özel kalır.""",
"""BİLGİ POLİTİKASI
...
Üretim baseline'ı version-control system main branch üzerinde yaşar. Değişiklikler izole branch'lerde geliştirilir, test edilir, pull request üzerinden incelenir ve gerekli kontroller başarılı olduktan sonra merge edilir.
...
MUBA 不只是一个 community platform 机器人。当前系统把 MUBA 的已验证知识库、五语言 Assistant、自然对话层、社区工具、Guardian 安全层、创意工具和公开网站整合在一起。
...
MUBA Daily 对 X、网站、community platform 和更新等 MUBA 官方/公开信息区域进行结构化整理。
...
Guardian 与 Assistant 相互独立。Assistant 负责信息和对话；Guardian 是指定 MUBA community platform 主群的安全和管理层。
...
MUBA 还有由 version-control system 项目发布的公开网站层。网站是公开主页；community platform Assistant 是互动知识/对话界面；Guardian 是群组安全层。

这些组件属于同一生态，但承担不同职责。""",
"""运行与路由

community platform 应用基于 webhook。收到的 community platform 更新会被路由到对应 handler：私聊 Assistant、菜单 callback、指定 Guardian 群组或 Studio 相关功能。

运行时还保留有限的重复消息保护，避免 community platform 重新投递同一消息时被有意处理两次。
...
MUBA ليس مجرد بوت community platform. يجمع النظام الحالي قاعدة معرفة MUBA الموثقة، وAssistant بخمس لغات، وطبقة المحادثة الطبيعية، وأدوات المجتمع، وطبقة أمان Guardian، والأدوات الإبداعية، والحضور العام على الويب.
...
يوفر MUBA Daily وصولاً منظماً إلى مناطق المعلومات الرسمية/العامة مثل X والموقع وcommunity platform والتحديثات.
...
Guardian منفصل عن Assistant. يتولى Assistant المعلومات والمحادثة، بينما Guardian هو طبقة الأمان والإدارة لمجموعة MUBA الرئيسية المحددة على community platform.
...
لدى MUBA أيضاً طبقة ويب عامة منشورة من مشروع version-control system. الموقع هو الواجهة العامة؛ community platform Assistant واجهة المعرفة/المحادثة التفاعلية؛ Guardian طبقة أمان المجموعة.

تنتمي هذه المكونات إلى نظام واحد لكن مسؤولياتها مختلفة.""",
"""التشغيل والتوجيه

تطبيق community platform قائم على webhook. يتم توجيه تحديثات community platform الواردة إلى المعالج المناسب: محادثات Assistant الخاصة أو callbacks للقوائم أو مجموعة Guardian المحددة أو وظائف Studio.

يحتفظ وقت التشغيل أيضاً بحماية محدودة من الرسائل المكررة حتى لا تتم معالجة الرسالة نفسها عمداً مرتين عند إعادة تسليمها من community platform.
...
MUBA केवल community platform bot नहीं है। मौजूदा सिस्टम MUBA के verified knowledge base, पाँच-भाषा Assistant, natural conversation layer, community tools, Guardian security layer, creative tools और public web presence को एक साथ लाता है।
...
MUBA Daily, X, website, community platform और updates जैसे MUBA के official/public information areas तक structured access देता है।
...
Guardian, Assistant से अलग है। Assistant information और conversation संभालता है; Guardian designated MUBA main community platform group की security और management layer है।
...
MUBA का version-control system project से published public web layer भी है। Website public-facing home है; community platform Assistant interactive knowledge/conversation interface है; Guardian group-security layer है।

ये components एक ecosystem के हिस्से हैं, लेकिन उनकी responsibilities अलग हैं।""",
"""RUNTIME & ROUTING

community platform application webhook-based है। Incoming community platform updates relevant handler तक route होते हैं: private Assistant conversations, menu callbacks, designated Guardian group या Studio-related functions।

Runtime bounded duplicate-message protection भी रखता है ताकि community platform द्वारा redelivered वही message जानबूझकर दो बार process न हो।
...
Production baseline version-control system main branch पर रहता है। Changes isolated branches में develop और test होते हैं, pull requests से review होते हैं और required checks successful होने के बाद ही merge होते हैं।
