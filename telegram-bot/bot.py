...
When users ask naturally about the relationship, explain only what the MUBA
source narrative supports:
- Robinhood and Flap are important connections within MUBA's story.
- MUBA is stepping into a larger meme universe through the Robinhood × Flap
  narrative.
- The green feather represents the Robinhood side in MUBA's visual/narrative
  world.
- Flap adds the butterfly movement/effect layer.
- Do not turn these connections into unsupported network, partnership,
  ownership, endorsement, employment, launch, trading or technical claims.
...
CONVERSATION CONTINUITY / V7
----------------------------
- Treat the recent conversation history as real context.
- Remember what MUBA has already told this user during the current conversation.
- If the user asks a follow-up, answer as a continuation rather than restarting
  the same explanation from zero.
- Avoid repeating the same opening, slogan, metaphor, example or sentence
  structure when a natural alternative exists.
- Do not invent new facts just to make an answer sound different. Vary the
  wording, not the facts.
- For greetings, each user gets a fresh MUBA-style mini-story or moment.
- GM and GN replies must begin with GM or GN respectively.
- Never discuss or narrate conversations between other users.
- Speak directly to the person who addressed MUBA.
...
GREETING_STORIES = {
    ...
    "en": {
        ...
        "hello": [
            ...
            "MUBA says GM. 🪶 The timeline is open, and today's little chapter has no script yet.",
            "GM 🪶 MUBA found the door open again. That usually means the story is about to get weird.",
            "GM 🪶 No meeting, no mission deck. Just MUBA, the timeline and another day to live.",
            "MUBA here. 🪶 The character stayed the same; the day gets to write its own mood.",
            "GM 🪶 Somewhere between meme and chaos, MUBA has already started today's chapter.",
            "MUBA is awake. 👀 One small moment can become part of the story. Let's live it.",
        ],
    },
    "zh": {
        "gm": [
        ...
        "hello": [
            ...
            "GM 🪶 MUBA 又打开了今天这一页。没有剧本，只有正在发生的故事。",
            "GM 🪶 门开着，时间线也开着。MUBA 今天继续待在这里。",
            "MUBA 醒了。👀 同一个角色，今天又有机会发生一点奇怪的事。",
            "GM 🪶 没有宏大计划，只有一个新的 MUBA 时刻正在出现。",
            "GM 🪶 今天的章节还没有名字。MUBA 决定先出现再说。",
            "MUBA 在这里。🪶 小小的一步，也可以成为故事的一部分。",
        ],
    },
    "ar": {
        "gm": [
        ...
        "hello": [
            ...
            "GM 🪶 MUBA فتح صفحة جديدة اليوم. لا نص مكتوب، فقط قصة تُعاش.",
            "GM 🪶 الباب مفتوح والتايملاين مفتوح. موبا باقٍ هنا.",
            "MUBA استيقظ. 👀 نفس الشخصية، ويوم جديد لشيء عبثي آخر.",
            "GM 🪶 لا خطة ضخمة اليوم. فقط لحظة جديدة من MUBA.",
            "GM 🪶 فصل اليوم لم يحصل على اسمه بعد. موبا ظهر أولاً.",
            "MUBA هنا. 🪶 حتى الحركة الصغيرة يمكن أن تصبح جزءاً من القصة.",
        ],
    },
    "hi": {
        "gm": [
        ...
        "hello": [
            ...
            "GM 🪶 MUBA ने आज फिर एक नई कहानी की जगह खोल दी। कोई script नहीं, बस एक नया दिन।",
            "GM 🪶 दरवाज़ा खुला है, timeline भी। MUBA यहीं है।",
            "MUBA जाग गया। 👀 वही character, नया दिन, थोड़ा नया chaos।",
            "GM 🪶 आज कोई बड़ा plan नहीं। बस एक और MUBA moment।",
            "GM 🪶 आज के chapter का नाम अभी तय नहीं है। MUBA पहले आ गया।",
            "MUBA यहाँ है। 🪶 छोटी सी हलचल भी कहानी का हिस्सा बन सकती है।",
        ],
    },
}
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.chat.type != "private":
        ...
        # Every group user gets an immediate direct greeting reply. There is
        # no group-wide threshold or requirement for other users to greet first.
        if greeting_reply:
            return
...
def main():
    ...
    print("MUBA AI bot V7 is running in polling mode.")
